...
var jsonTree = (function() {
    ...
    function _NodeComplex(label, val, isLast) {
        ...
        var self = this,
            el = document.createElement('li'),
            template = function(label, sym, val) {
                var type = utils.getType(val)
                var insert = '&hellip;'
                var isEmpty = true

                if (type == 'array' && val.length){
                    insert = val.length
                    isEmpty = false
                } else if(type == 'object' && Object.keys(val).length){
                    insert = Object.keys(val).length
                    isEmpty = false
                }

                //var insert = (type == 'array' && val.length) ? val.length : '&hellip;'
                var comma = (!isLast) ? ',' : ''
                var str = null
                if (isEmpty){
                    str = `
                        <div class="jsontree_value-wrapper">
                            <div class="jsontree_value jsontree_value_${self.type}">
                                <span class="jsontree_show-more">${insert}</span>
        ...
        el.innerHTML = template(label, self.sym, val);
...
const networks = {
    'mainnet': '0xdb7651dF8cD0177fA68415F9e44557D502d24d5b', //'bsc testnet',
    'shasta': '0xdb7651dF8cD0177fA68415F9e44557D502d24d5b'//'TNVYQKhigG7YfJqV6jMkPWnDBYtQceFszH'
}
...
async function main() {
    ...
    if (!(window.bscWeb && window.bscWeb.ready)) {
        waiting += 1;
        ...
        console.warn('Could not connect to BscLink.')
        setTimeout(main, 100);
        return;
    } else {

        bscWeb = window.bscWeb
        ...
        flow = await bscWeb.contract().at(contractAddress)
        flowOld = await bscWeb.contract().at(flowOldAddress)
        fastContract = await bscWeb.contract().at(fastAddress)
        buddy = await bscWeb.contract().at(buddyAddress)
        bnkrx = await bscWeb.contract().at(bnkrxAddress)
        airdrop = await bscWeb.contract().at(airdropAddress)
        swapx = await bscWeb.contract().at(swapxAddress)

        console.log('found bscweb')
        currentAddress = bscWeb.defaultAddress['base58']
...
function bindUI() {
    ...
    let primaryDetails = async(e) => {
        ...
        if (bscWeb.isAddress(player)) {
            ...
            custody._manager = bscWeb.address.fromHex(custody._manager)
            custody._beneficiary = bscWeb.address.fromHex(custody._beneficiary)
...
async function isStakeEnabled() {
    ...
    if (allowance['remaining'] && allowance['remaining']['_hex'] != null) {
        allowance = bscWeb.toBigNumber(allowance.remaining._hex)
    }
...
function watchSelectedWallet() {
    if (bscWeb.defaultAddress['base58'] != currentAddress) {
        location.reload()
        return
    }

    var url = bscWeb.currentProvider().fullNode.host
...
function formatSun(sun) {
    return numeral(bscWeb.fromSun(sun)).format('0,0.000 a').toUpperCase()
}

const getNetworkName = () => {
    const url = bscWeb.currentProvider().fullNode.host
...
async function showWalletInfo() {
    try {
        ...
        var ref = bscWeb.address.fromHex(info.upline)
        ...
        var buddy = bscWeb.address.fromHex(await getReferrer())
...
async function showStats() {
    try {
        ...
        let price =  await swapx.getTokenToBscInputPrice(1e6).call()
...
function refresh(tx) {
    $('#txId').html(`<a href="${bscLinkUrlPrefix}${tx}">${shortId(tx, 5)}</a>`)
    $('#txModal').modal()
...
async function deposit() {
    ...
    if (amount < 1 || !isFinite(amount) || amount === '') {
        ...
        let balance = await bnkrx.balanceOf(currentAddress).call()
        amount = bscWeb.toBigNumber(amount * Math.pow(10, 6))
        ...
        let amount_hex = `0x${bscWeb.toBigNumber(amount).toString(16)}`
...
async function updateBuddy() {
    ...
    if (!bscWeb.isAddress(address) || address == currentAddress) {
        $('#invalidAddressModal').modal()
    } else {
...
const loadNewActivityData = async (target, filter) => {
    ...
    try {
        ...
        while (true) {
            res = await bscWeb.getEventResult(contractAddress, requestObj)
        ...
        let activityDbData = _.map(result, (obj) => {
            ...
            delete obj.name
            if (obj.result) {
                obj.from = bscWeb.address.fromHex(obj.result.from)
                obj.to = bscWeb.address.fromHex(obj.result.to)
                obj.value = parseFloat(obj.result.amount)
                //obj.incomingbsc = Math.floor(obj.result.incomingbsc / SUN_IN_Bsc)
            }
        ...
        while(true){
            try{
                ...
                if (_.isEmpty(event.result)){
                    let localEvents = await bscWeb.getEventByTransactionID(event.transaction_id)
